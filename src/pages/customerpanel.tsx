...
import { CheckCircle2, Printer, User, Car, Droplets, MapPin, Phone, Plus, Trash2 } from "lucide-react";
...
const CustomerPanel = () => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>My Emergency QR - ${activatedCode}</title>
          <style>
            body {
            ...
            @media print {
              body { margin: 0; }
              .card { box-shadow: none; border: 3px solid #dc2626; }
            }
          </style>
        </head>
        <body>
    ...
    <div className="min-h-screen bg-background p-4 max-w-md mx-auto space-y-4">
      ...
      <div className="text-center py-4">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Fill your details → Pay ₹{QR_PRICE} → Get instant QR code
        </p>
      </div>
      ...
      <Card className="border-primary bg-primary/5">
        <CardContent className="p-3 text-center">
          ...
          <p className="text-3xl font-bold text-primary">₹{QR_PRICE}</p>
      ...
      <Card className="card-shadow">
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input placeholder="Ramesh Kumar" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          </div>
          <div>
            <Label>Vehicle Number *</Label>
            <Input placeholder="MP20SM9609" value={form.vehicle_number} onChange={(e) => setForm({ ...form, vehicle_number: e.target.value.toUpperCase() })} />
          </div>
          <div>
          ...
          <div>
            ...
            <Input placeholder="123, MG Road, Jabalpur" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          {contacts.map((contact, i) => (
            <div key={i} className="border rounded-lg p-3 space-y-2 relative">
              {contacts.length > 1 && (
                <button onClick={() => removeContact(i)} className="absolute top-2 right-2 text-destructive">
                  <Trash2 size={14} />
                </button>
              )}
      ...
      >
        {registerMutation.isPending ? "Activating..." : `Pay ₹${QR_PRICE} & Get QR Code`}
      </Button>
